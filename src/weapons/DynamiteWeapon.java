...
public class DynamiteWeapon extends WeaponDef {
    ...
    public void fire(Player player, int level, List<Enemy> enemies, GameController controller) {
        ...
        // spawn a simple slow projectile that moves toward target position and explodes on impact or end of life
        ...
        Projectile p = new Projectile(player, controller.nodoActual, player.x, player.y, 0f, 0f, life, 0, controller) {
            ...
            public void update(float dt, GameController ctx) {
                if (!initialized) {
                    ...
                    if (finalTarget != null && finalTarget.isAlive()) { tx = finalTarget.getX(); ty = finalTarget.getY(); }
                ...
                if (roomNode != null) {
                    ...
                    for (Enemy en : list) {
                        ...
                        if (dx*dx + dy*dy <= 14f*14f) {
                            // explode
                            ctx.spawnEffect(new AoEExplosion(player, roomNode, x, y, 48f + level*6f, damage, 0.6f, ctx));
                            expire();
                            return;
                        }
                    }
                ...
                life -= dt;
                if (life <= 0f) {
                    controller.spawnEffect(new AoEExplosion(player, roomNode, x, y, 48f + level*6f, damage, 0.8f, controller));
                    expire();
                }
            }
